...
#include "pch.h"
#include "TraceLogging.TraceLoggingCommon.g.h"
#include "TraceLoggingCommon.h"
#include "TraceLogging.TraceLoggingCommon.g.cpp"
...
namespace winrt::TraceLogging::implementation
{
    TraceLoggingCommon::TraceLoggingCommon()
        : g_calculatorProvider(
            ...
            LoggingChannelOptions(
                winrt::guid(0x4f50731a, 0x89cf, 0x4782, std::array<uint8_t, 8>({0xb3, 0xe0, 0xdc, 0xe8, 0xc9, 0x4, 0x76, 0xba }))),
            // Unique providerID {0905CA09-610E-401E-B650-2F212980B9E0}
            winrt::guid(0x905ca09, 0x610e, 0x401e, std::array<uint8_t, 8>({0xb6, 0x50, 0x2f, 0x21, 0x29, 0x80, 0xb9, 0xe0 })))
        , m_appLaunchActivity({ nullptr })
    {
        CoCreateGuid(&sessionGuid);
    }

    TraceLogging::TraceLoggingCommon TraceLoggingCommon::GetInstance()
    {
        auto factory = winrt::make<factory_implementation::TraceLoggingCommon>();
    ...
    void TraceLoggingCommon::LogLevel1Event(winrt::hstring const& eventName, winrt::Windows::Foundation::Diagnostics::LoggingFields const& fields)
    {
    ...
    void TraceLoggingCommon::LogLevel2Event(winrt::hstring const& eventName, winrt::Windows::Foundation::Diagnostics::LoggingFields const& fields)
    {
        //if (!GetTraceLoggingProviderEnabled())
        //{
        //    return;
    ...
    void TraceLoggingCommon::LogLevel3Event(winrt::hstring const& eventName, winrt::Windows::Foundation::Diagnostics::LoggingFields const& fields)
    {
        //if (!GetTraceLoggingProviderEnabled())
        //{
        //    return;
